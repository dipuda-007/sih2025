...
<head>
  ...
  <title>Health Centre Sign Up</title>
  <style>
    body {
      margin: 0;
...
<body>
  ...
  if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $sql="SELECT * FROM `healthcenterlogin` WHERE `username` = '$username'";
    $result=mysqli_query($conn,$sql);
    $num=mysqli_num_rows($result);
    if($num==0){
      if($password==$upassword){
        $sql1="INSERT INTO `healthcenterlogin` ( `name`, `email`, `password`, `username`) VALUES ( '$name', '$email', '$password', '$username')";
  ...
  ?>
    ...
    <div class="login-text">
      Already have an account? <a href="health_login.php">Login</a>
    </div>
  </div>
